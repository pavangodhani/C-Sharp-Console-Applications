...
namespace GradeBook.UserInterfaces
{
    public static class GradeBookUserInterface
    {
        ...
        private static void AddStudentCommand(string command)
        {
            ...
            if (parts.Length != 4)
            {
                Console.WriteLine("Command not valid, Add requires a student name, student type, enrollment type.");
                return;
            }
            ...
            if (!Enum.TryParse(parts[3], true, out studentEnrollmentType))
            {
                Console.WriteLine("{0} is not a valid enrollment type, try again.", parts[3]);
                return;
            }

            var student = new Student(studentName, studentType, studentEnrollmentType);
